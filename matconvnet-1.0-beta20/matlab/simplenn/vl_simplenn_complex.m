function res = vl_simplenn_complex(net, x, dzdy, res, varargin)
...
for i=1:n
  ...
  switch l.type
    case 'conv'
%       res(i+1).x = vl_nnconv(res(i).x, l.weights{1}, l.weights{2}, 'pad', l.pad, 'stride', l.stride, l.opts{:}, cudnn{:}) ;

      rr = vl_nnconv(real(res(i).x), real(l.weights{1}), real(l.weights{2}), 'pad', l.pad, 'stride', l.stride, l.opts{:}, cudnn{:}) ;
      ri = vl_nnconv(real(res(i).x), imag(l.weights{1}), imag(l.weights{2}), 'pad', l.pad, 'stride', l.stride, l.opts{:}, cudnn{:}) ;
      ir = vl_nnconv(imag(res(i).x), real(l.weights{1}), real(l.weights{2}), 'pad', l.pad, 'stride', l.stride, l.opts{:}, cudnn{:}) ;
      ii = vl_nnconv(imag(res(i).x), imag(l.weights{1}), imag(l.weights{2}), 'pad', l.pad, 'stride', l.stride, l.opts{:}, cudnn{:}) ;

      res(i+1).x    = (rr-ii) + 1i*(ri+ir);
    case 'convt'
...
%       res(i+1).x = vl_nnrelu(res(i).x,[],leak{:}) ;
      rr = vl_nnrelu(real(res(i).x));
      ii = vl_nnrelu(imag(res(i).x));
...
%         res(i+1).x = vl_nnbnorm(res(i).x, l.weights{1}, l.weights{2}, 'moments', l.weights{3}) ;
        rr = vl_nnbnorm(real(res(i).x), real(l.weights{1}), real(l.weights{2}), 'moments', l.weights{3}) ;
        ii = vl_nnbnorm(imag(res(i).x), imag(l.weights{1}), imag(l.weights{2}), 'moments', l.weights{3}) ;
        
        res(i+1).x = rr + 1i*ii;
      else
%         res(i+1).x = vl_nnbnorm(res(i).x, l.weights{1}, l.weights{2}) ;
        rr = vl_nnbnorm(real(res(i).x), real(l.weights{1}), real(l.weights{2})) ;
        ii = vl_nnbnorm(imag(res(i).x), imag(l.weights{1}), imag(l.weights{2})) ;
    ...
    case 'concat'
      res(i+1).x = cat(3, res([l.concat, i]).x);
...
if doder
  ...
  for i=n:-1:backPropLim
    ...
    switch l.type
      ...
      case 'conv'
%         [res(i).dzdx, dzdw{1}, dzdw{2}] = vl_nnconv(res(i).x, l.weights{1}, l.weights{2}, res(i+1).dzdx, 'pad', l.pad, 'stride', l.stride, l.opts{:}, cudnn{:}) ;
        [rr_dzdx, rr_dzdw1, rr_dzdw2]	= vl_nnconv(real(res(i).x), real(l.weights{1}), real(l.weights{2}), real(res(i+1).dzdx), 'pad', l.pad, 'stride', l.stride, l.opts{:}, cudnn{:}) ;
        [ri_dzdx, ri_dzdw1, ri_dzdw2]	= vl_nnconv(-imag(res(i).x), -imag(l.weights{1}), -imag(l.weights{2}), real(res(i+1).dzdx), 'pad', l.pad, 'stride', l.stride, l.opts{:}, cudnn{:}) ;
        [ir_dzdx, ir_dzdw1, ir_dzdw2]	= vl_nnconv(real(res(i).x),	real(l.weights{1}), real(l.weights{2}), -imag(res(i+1).dzdx), 'pad', l.pad, 'stride', l.stride, l.opts{:}, cudnn{:}) ;
        [ii_dzdx, ii_dzdw1, ii_dzdw2]	= vl_nnconv(-imag(res(i).x), -iamg(l.weights{1}), -imag(l.weights{2}), -imag(res(i+1).dzdx), 'pad', l.pad, 'stride', l.stride, l.opts{:}, cudnn{:}) ;

        res(i).dzdx = (rr_dzdx - ii_dzdx) + 1i*(ri_dzdx + ir_dzdx);
        dzdw{1} 	= (rr_dzdw1 - ii_dzdw1) + 1i*(ri_dzdw1 + ir_dzdw1);
        dzdw{2}     = (rr_dzdw2 - ii_dzdw2) + 1i*(ri_dzdw2 + ir_dzdw2);
        
        res(i).dzdw{1} = zeros(size(dzdw{1}));
        res(i).dzdw{2} = zeros(size(dzdw{2}));
...
%           res(i).dzdx = vl_nnrelu(res(i).x, res(i+1).dzdx, leak{:}) ;
          rr = vl_nnrelu(real(res(i).x), real(res(i+1).dzdx), leak{:}) ;
          ii = vl_nnrelu(imag(res(i).x), imag(res(i+1).dzdx), leak{:}) ;
...
%           res(i).dzdx = vl_nnrelu(res(i+1).x, res(i+1).dzdx, leak{:}) ;
          rr = vl_nnrelu(real(res(i+1).x), real(res(i+1).dzdx), leak{:}) ;
          ii = vl_nnrelu(imag(res(i+1).x), imag(res(i+1).dzdx), leak{:}) ;
...
%         dzdw{3} = dzdw{3} * size(res(i).x,4) ;
    ...
    [rr_dzdx, rr_dzdw1, rr_dzdw2, rr_dzdw3] = vl_nnbnorm(real(res(i).x), real(l.weights{1}), real(l.weights{2}), real(res(i+1).dzdx)) ;
    [ii_dzdx, ii_dzdw1, ii_dzdw2, ii_dzdw3] = vl_nnbnorm(imag(res(i).x), imag(l.weights{1}), imag(l.weights{2}), imag(res(i+1).dzdx)) ;
    
    res(i).dzdx = rr_dzdx + 1i*ii_dzdx;
    dzdw{1}     = rr_dzdw1 + 1i*ii_dzdw1;
    dzdw{2}     = rr_dzdw2 + 1i*ii_dzdw2;
    dzdw{3}     = rr_dzdw3 + 1i*ii_dzdw3;
    
    res(i).dzdw{1} = zeros(size(dzdw{1}));
    res(i).dzdw{2} = zeros(size(dzdw{2}));
    res(i).dzdw{3} = zeros(size(dzdw{3}));
