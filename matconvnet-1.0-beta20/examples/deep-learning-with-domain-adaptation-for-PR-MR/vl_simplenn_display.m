...
for j = 1:numel(net.layers)
    ly = net.layers{j};
    if (strcmp(ly.type, 'send'))
        for k = 1:length(ly.send)
            send{ly.send(k)} = [send{ly.send(k)} j];
        end
    elseif (strcmp(ly.type, 'receive'))
        for k = 1:length(ly.receive)
            receive{ly.receive(k)} = [receive{ly.receive(k)} j];
        end
    elseif (strcmp(ly.type, 'concat'))
        for k = 1:length(ly.concat)
            concat{ly.concat(k)} = [concat{ly.concat(k)} j];
        end
    end
end
...
for l = 1:numel(net.layers)
  ...
  switch ly.type
    ...
    case 'unpool'
        info.dataSize(1, l+1) = info.dataSize(1, l+1)*info.support(1,l)*info.stride(1,l);
        info.dataSize(2, l+1) = info.dataSize(2, l+1)*info.support(2,l)*info.stride(2,l);
    case 'concat'
       	info.dataSize(3, l+1) = info.dataSize(3,l) + sum(info.dataSize(3,[send{ly.concat}]));
