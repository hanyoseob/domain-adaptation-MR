...
for i=1:n
  ...
  switch l.type
    ...
    case 'euclideanloss_mmd'
      res(i+1).x = vl_nneuclideanloss_mmd(res(i).x, l.sgm, l.class) ;
      
    case 'euclideanloss_mmd_lp'
      res(i+1).x = vl_nneuclideanloss_mmd_lp(res(i).x, l.p, l.sgm, l.class) ;
      
    case 'euclideanloss_mmd_log'
      res(i+1).x = vl_nneuclideanloss_mmd_log(res(i).x, l.p, l.sgm, l.class) ;
...
%       for j = 1:i-1
%           l_ = net.lauers{j};
%           if (strcmp(l_.type, 'send') & find(l_.send == l.receive))
%             res(i+1).x = res(i+1).x + res(j).x;
%           end
%       end
...
%       for j = 1:i-1
%           l_ = net.layers{j};
%           if (strcmp(l_.type, 'send') & find(l_.send == l.concat))
%               concat_ = [concat_ j];
%           end
%       end
%       concat_ = [concat_ i];
%       res(i+1).x = cat(3, res(concat_).x);
...
if doder
  ...
  for i=n:-1:backPropLim
    ...
    switch l.type
      ...
      case 'euclideanloss_mmd'
        res(i).dzdx = vl_nneuclideanloss_mmd(res(i).x, l.sgm, l.class, res(i+1).dzdx) ;
        
      case 'euclideanloss_mmd_lp'
        res(i).dzdx = vl_nneuclideanloss_mmd_lp(res(i).x, l.p, l.sgm, l.class, res(i+1).dzdx) ;
                
      case 'euclideanloss_mmd_log'
        res(i).dzdx = vl_nneuclideanloss_mmd_log(res(i).x, l.p, l.sgm, l.class, res(i+1).dzdx) ;
